...
//Configure::load('myconfig');
//echo Configure::read('MyConf.secret');
...
//echo "Deleted Successfully";
    ...
    public function displayaddondeatils() {  // function to get add-on course list
        ...
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/getaddondesc.php', false, $context1);
    ...
    public function displayaddondeatilsnew() { // function to get semesters list
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/getaddondescnew.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        $getdesc = $topiccodes[0]['description'];
        $name = $topiccodes[0]['name'];
    ...
    public function index() { // index page functions
        ...
        $photoname1 = $this->Studentgroup->find('all', ['conditions' => ['Studentgroup.id' => $arr]])->first();
        $comp.=$photoname1->compareacode.',';
        }
        $ff = rtrim($comp, ',');
        
        $fcomps = $this->dupli($ff);
                  $this->request->getSession()->write('fcomps', $fcomps);
      ...
      //  $loggesingroupid = $this->request->getSession()->read('groupid');
       ...
       // $useridmoodle = $this->request->getSession()->read('moodleuid');
		...
		$exp_country = $degreetype->exp_country;
  ...
  $studentgroups = $studentgroups1->fetchAll(\PDO::FETCH_ASSOC);
            ...
            foreach ($getpaymentdetails as $det) {
				...
				if($det['durartion'] != 0) {
							...
							if(strtotime($det['datetime']) < strtotime(''.$dur.' days ago'))
{
	
	$moducode = $det['productcode'];	
		$modulkeid = $det['id'];	


		  		   $row = array('moduleid' =>  $moducode);
        $data = json_encode($row);
        $options1 = array(
            'http' => array(
        ...
        $Targetusers = $this->Target->find('all', array('fields' => array('examcode', 'id')))->where(['Target.users_id' => $this->request->getSession()->read('sessionname'), 'Target.examcode' => $selectedOption]);
        ...
        foreach ($Targetusers as $tar) {
            ...
            $query = $optionalblockTable->query();
            $query->delete()
                    ->where(['target_id' => $modulkeid])
                    ->execute();
            $optionalblockTable = TableRegistry::get('events');
            $query = $optionalblockTable->query();
            $query->delete()
                    ->where(['targetidevent' => $modulkeid])
                    ->execute();
...
$conn = ConnectionManager::get('default');
           ...
           /* $noticationscount = TableRegistry::get('notications')->find('all')
                 ...
                 $input = '0';
$list = $this->request->session()->read('groupid').',46';
...
$locQuery = implode(' OR ', array_map(function($locArray) { return "FIND_IN_SET($locArray, groups)"; }, $locArray));
            ...
            $querytcomps = "SELECT * FROM notications where ($locQuery) and notificationdate ='".$curdate."' ORDER BY id DESC";




           $stmt1 = $conn->execute($querytcomps);
	  $noticationscount = $stmt1->rowCount();
...
$locQuery = implode(' OR ', array_map(function($locArray) { return "FIND_IN_SET($locArray, groupid)"; }, $locArray));
            ...
            $querytcomps = "SELECT id FROM users where ($locQuery) and userroles_id = 2  and status=1 and  groupid != 0 ORDER BY id DESC";
  ...
  $studentgroups = $studentgroups1->fetchAll(\PDO::FETCH_ASSOC);
        ...
        if ($this->request->getSession()->read('sessionname2') == 3) {
            ...
            /*$userlistsfinal = $this->Users->find('all', array('fields' => array('id', 'email', 'fname', 'lname'), 'order' => 'Users.fname asc'))->where(['Users.status' => 1, 'Users.userroles_id' => 2, 'Users.groupid' => 0]);*/
             $userlistsfinal = $this->Users->find('all', array('fields' => array('id', 'email', 'fname', 'lname'), 'order' => 'Users.fname asc'))->where(['Users.status' => 1, 'Users.userroles_id' => 2]);
  ...
  public function saveuserdatailsexpe() {  // function to save profile information
        if ($this->request->is(array('ajax'))) {
            //  $areaofinterest = $this->Users->newEntity();
            $areaofinterest = $this->Users->get($this->request->getSession()->read('sessionname'), [
                'contain' => []
            ...
            $areaofinterest->exp_industry = $this->request->data['exp_industry'];
            $areaofinterest->exp_expertise = $this->request->data['exp_expertise'];
            $areaofinterest->exp_experience = $this->request->data['exp_experience'];
            $areaofinterest->exp_currentcompany = $this->request->data['exp_currentcompany'];
            $areaofinterest->exp_namecurrentcompany = $this->request->data['exp_namecurrentcompany'];
            $areaofinterest->exp_website = $this->request->data['exp_website'];
            $areaofinterest->exp_country = $this->request->data['exp_country'];
            $areaofinterest->exp_levels = $this->request->data['exp_levels'];
			$areaofinterest->digitaltransformation = $this->request->data['digitaltransformation'];
			$areaofinterest->workindustry = $this->request->data['workindustry'];
			$areaofinterest->dtprojects = $this->request->data['dtprojects'];
    ...
    public function updatestutsoflesson() { //skip function
       ...
       public function removegroupfromthr() { //skip function
        ...
        if ($this->request->is(array('ajax'))) {
            ...
            $this->loadModel('Studentgroup');
                ...
                $photoname1 = $this->Studentgroup->find('all', ['conditions' => ['Studentgroup.id' => $id]])->first();
        $photoname = $photoname1->compareacode;
            
             $final = $this->removeFromString($photoname, $cat_val);

            $this->Studentgroup->updateAll(['compareacode' => $final], ['id' => $id]);
          //  echo $feed;
        }
    }
...
$locQuery = implode(' OR ', array_map(function($locArray) { return "FIND_IN_SET($locArray, groupid)"; }, $locArray));
            ...
            $querytcomps = "SELECT * FROM users where ($locQuery) and userroles_id = 2  and status=1 and  groupid != 0 ORDER BY fname ASC";
            ...
            while ($row1 = $stmt1->fetch(\PDO::FETCH_ASSOC)) {
              ...
              //  $groupid = $this->getgroupname($groupid);
                  ...
                  $newbreak = explode(",",$groupid);
                
                $getvalue ="";
                foreach($newbreak as $newv)
                {
                    $getvalue.=$this->getgroupname($newv).',';
                }
                
                $groupid =  rtrim($getvalue, ',');
                ...
                $post_item1 = array(
					...
					'markscard' => html_entity_decode("<a style='cursor:pointer'  onclick='assignmarkesforstude($id)'>View  </a>"),
					  'attendance' => html_entity_decode("<a style='cursor:pointer'  onclick='attandacestuiwise($id)'>View  </a>"),
                    ...
                    'assignments' => html_entity_decode("<a style='cursor:pointer'  onclick='showallasignemntsforst($id)'>View</i>  </a>"),
    ...
    public function getscoreforbaselinegetscore($useridmoodle) { //functions to calculate total and pending hours
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cms/totalaveragescore.php', false, $context1);
      ...
      public function studentslist() { //student info for admin
        ...
        if ($this->request->is(array('ajax'))) {
            ...
            $querytcomps = 'SELECT * FROM users where (fname LIKE "%' . $prod_cat . '%" or lname LIKE "%' . $prod_cat . '%" or username LIKE "%' . $prod_cat . '%") and userroles_id = 2  ORDER BY fname ASC';
            ...
            while ($row1 = $stmt1->fetch(\PDO::FETCH_ASSOC)) {
             ...
             //   $groupid = $this->getgroupname($groupid);
                ...
                $post_item1 = array(
                    ...
                    'resetpass' => html_entity_decode("<a style='cursor:pointer' onclick=rsetpasswordfromadmin($id) > Reset  </a>")
                );
       ...
       public function changepassforstudentadmin() { //profile change password
...
INNER JOIN targetcomps tc
 ...
 WHERE tc.target_id IN (' . $prod_cat . ')';
	...
	public function getteachername($gid)
	{
		 $conn = ConnectionManager::get('default');
        $stmt = $conn->execute('SELECT fname,lname FROM users WHERE FIND_IN_SET(groupid, "'.$gid.'")');

        $posts_arr = array();
		$table =" | Group Administrator(s) :";
...
FROM  targetcomps tc WHERE tc.target_id IN (' . $Targetusers . ') and lesson = ' . $lessonid . ' GROUP BY tc.topiccode order by tc.level asc';
    ...
    public function gettargetcompsid($courseid) { //return targtecomps ids
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/gettargetcompsid.php', false, $context1);
        $topiccodes = json_decode($result1, true);
    ...
    public function getexamcount($a) { //get target count
        ...
        /* 	var_dump($options1);
          exit; */
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/getcount.php', false, $context1);
        $topiccodes = json_decode($result1, true);
    ...
    public function gettopicsfromcmd() { //topic code count
        ...
        /* 	var_dump($options1);
          exit; */
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getalltopiccount.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        return $topiccodes;
    }
    ...
    public function disaplyallmodulecourses() { //return modules
        ...
        /* 	var_dump($options1);
          exit; */
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/modulesdisplay.php', false, $context1);
        $topiccodes = json_decode($result1, true);
    ...
    public function disaplyalladdoncourses() { //return addon courses
        ...
        if ($parameters['pass'][0] != NULL) {
            ...
            $context1 = stream_context_create($options1);
            $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/addoncoursedisplay.php', false, $context1);
						...
						if($tctype =='SEM')
{
	...
	$scorecard->color = 'orange';
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'sms/getsamenamescom.php', false, $context1);
        $topiccodes = json_decode($result1, true);
	...
	public function getmoodlecomplestatusfinal1($courseid,$moodleid) {
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'sms/getsamenames.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        return $topiccodes[0]['name'];
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'sms/getsamenamesnew.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        return $topiccodes[0]['name'];
            ...
            if (strlen($this->request->data('phonenumber')) != 10) {
                ...
                if ($this->Users->save($user)) {
				   ...
				   //$this->emailadressconfimation($_POST['email'], $_POST['fname']);
                    ...
                    return $this->redirect(Configure::read('MyConf.weburlmainsite') . 'users/addga');
                }
            }
           // $this->Flash->error(__('The user could not be saved. Please, try again.'));
        }
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Users->save($user)) {
                ...
                return $this->redirect(Configure::read('MyConf.weburlmainsite') . 'users/userindex');
            }
            $this->Flash->error(__('The user could not be saved. Please, try again.'));
        }
    ...
    public function editga($id = null) {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Users->save($user)) {
                ...
                return $this->redirect(Configure::read('MyConf.weburlmainsite') . 'users/gausers');
            }
            $this->Flash->error(__('The user could not be saved. Please, try again.'));
    ...
    public function register() { // registration of user
        ...
        if ($this->request->is('post')) {
           ...
           /* $username = $this->request->data('username');
            ...
            if (strlen($fname) < '5' && Configure::read('Registration.firstname') == 1) {
          ...
          /*  $usern = $this->Users->find()
            ...
            if (strlen($this->request->data('phonenumber')) != 10 && Configure::read('Registration.phonenumber') == 1) {
                $this->Flash->error('Phone number should be 10 digits.');
            } else if ($query) {
...
// echo date("Y-m-d H:i:s");
        ...
        $userid = $this->request->getSession()->read('sessionname');
        $conn = ConnectionManager::get('default');
        $querytcomps = 'INSERT INTO eventlog (userid, type, time, val1, val2) VALUES ("' . $userid . '","' . $type . '","' . $paymentDate . '","' . $code . '","1");';
        $stmt1 = $conn->execute($querytcomps);
		// header("Location: ".$url);
		 //exit;
		return  $this->redirect($urlnn);
	}
  ...
  $this->Flash->success('You are logged out');
    ...
    public function addgroup() {
        ...
        if ($this->request->is('post')) {
            ...
            if (is_array($_POST["userlis"]) || is_object($_POST["userlis"])) {
                ...
                $groupid =  $this->dupli($blockt->groupid.','.$_POST["groupname"]);
                

                   $string = trim($groupid,",");
                    
                      $finalgroup = $this->removeFromString($string, 0);  
                    
                    
                    $blockt->groupid =  $finalgroup;
    ...
    public function getalltargetsforscorecard1() {
        ...
        /* 	var_dump($options1);
          exit; */
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getpracticeforscore.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        ...
        foreach ($myArray as $top) {
            ...
            foreach ($blocks as $level) {
                ...
                /* 	var_dump($options1);
                  exit; */
                $context1 = stream_context_create($options1);
                $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getquizesforscore.php', false, $context1);
        ...
        /* 	var_dump($options1);
          exit; */
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getpracticefinal.php', false, $context1);
        $topiccodes = json_decode($result1, true);
    ...
    public function getcoursetype($id) {
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getcoursetype.php', false, $context1);
        $topiccodes = json_decode($result1, true);
    ...
    public function getlessonidforscore($id) {
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getlessonidforcourse.php', false, $context1);
        $topiccodes = json_decode($result1, true);
    ...
    public function getcoursenamestudydurartion($id) {
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'cmd/getcoursestudyhours.php', false, $context1);
        $topiccodes = json_decode($result1, true);
...
//if($status==0){ $stat = "Disabled"; } else { $stat = "Active";}
    ...
    public function getsemname($id) {
	    ...
	    public function getmoduledurationfinal($id) {
        $row = array('id' => $id);
        $data = json_encode($row);
        $options1 = array(
            'http' => array(
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/getmodulesnamesdu.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        return $topiccodes[0]['name'];
        //var_dump( $topiccodes);
        //exit;
    ...
    public function getgraduationname($id) {
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/getgraduationanme.php', false, $context1);
    ...
    public function getgraduationnamefinaldata($id) {
        ...
        $context1 = stream_context_create($options1);
        $result1 = file_get_contents(Configure::read('MyConf.mainurl') . 'ims/getgraduationanmenew.php', false, $context1);
        $topiccodes = json_decode($result1, true);
        return $topiccodes[0]['name'];
        //var_dump( $topiccodes);
        //exit;
...
//$actionname ='<a href="#" style="cursor:pointer;" onclick="openmodalboxforlearning('.$tcode.','.$lecode.');return false;">Learning Modules</a>';
            ...
            //    foreach ($_POST["modules"] as $selectedOption) {
				...
				$selectedOption = $_POST["modules"];
                    ...
                    if ($selectedOption != NULL) {
                        ...
                        $blockt->productcode = $selectedOption;
                        $blockt->productname = $this->getsemname($selectedOption);
						  $blockt->durartion = $this->getmoduledurationfinal($selectedOption);
 ...
 $modules = $_POST["modules"];
                ...
                $compereanameslist = new TargetcompsController();
                $comlistforstudent = $compereanameslist->evalautemodsbyadmin($id, $moodleid,$modules);
...
public function privacy()
{
    ...
    public function getredirectionformos() {
        ...
        $pid = $_GET['pid'];
        $modules = $_GET['type'];
        $this->redirect(Configure::read('MyConf.weburlmainsite') . 'razor/pay.php?type=' . $modules . '&fjhqhjqew=' . $price1 . '&uid=' . $uid . '&name=' . $name . '&pid=' . $pid . '');
    }

    public function beforeFilter(Event $event) {
        $this->Auth->allow(['privacy','regsuccess','regfailed','studentcv', 'paymentpddf', 'pricing', 'emailconfirm', 'register', 'login', 'uploadfile', 'password', 'sendResetEmail', 'reset', 'logout', 'emailCheck', 'usernameCheck']);
    }
