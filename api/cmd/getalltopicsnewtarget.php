...
$resultlesson = $post->alltopicnewacasdeboc($prod_cat->compreaa);
// Get row count
...
if ($numlesson > 0) {
    ...
    while ($row = $resultlesson->fetch(PDO::FETCH_ASSOC)) {
        ...
        $post_item = array(
		  'topicid' => html_entity_decode($id),
		  'cmpid' => html_entity_decode($comparea_id),
            'comparea_id' => $post->compereanamereturn($comparea_id),
			'lessoncode' => $post->compereanamereturnlessoncode($id),
			'levelfortarget' => $post->getlevelforcomareafo($comparea_id),
