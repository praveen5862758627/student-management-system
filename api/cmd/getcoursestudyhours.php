...
$resultlesson = $post->getcoursenamestudyhours($prod_cat->id, 'quiz');
// Get row count
$numlesson = $resultlesson->rowCount();


if ($numlesson == 0) {
    $resultlesson = $post->getcoursenamestudyhours($prod_cat->id, 'lesson');
    // Get row count
    $numlesson = $resultlesson->rowCount();

    if ($numlesson == 0) {
        $resultlesson = $post->getcoursenamestudyhours($prod_cat->id, 'example');
        // Get row count
        $numlesson = $resultlesson->rowCount();
    }
}
