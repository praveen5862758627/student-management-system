...
<body class="grey lighten-3">
  ...
  <main class="pt-5 mx-lg-5">
    ...
    <h3><?= __('Lesson') ?><?= $this->Html->link(__('Add '), ['action' => '../lesson/add'],['class'=>'btn btn-primary']) ?></h3>
...
<table id="dtMaterialDesignExample" class="table table-striped table-bordered" cellspacing="0" width="100%">
		...
		<?php foreach ($notications as $notication): ?>
            <tr>
			  <?php $idd = $notication['id'];  $topic_id = $notication['topic_id']; ?>
			   <td><?php echo $notication['id']; ?></td>
			    <td><?php echo $this->Custom->gettopicname($topic_id); ?></td>
                <td><?php echo $notication['code']; ?></td>
				 
                <td><?php echo $notication['name']; ?></td>
               
                  
		<td><?php echo $notication['studyduration']; ?></td>
                <td class="actions">
				...
				<td> <?= $this->Html->link(__('Add '), ['action' => '../chapters/add', $idd],['class'=>'button']) ?></td>
				<td> <?= $this->Html->link(__('Add '), ['action' => '../quiz/add', $idd],['class'=>'button']) ?></td>
				<td> <?= $this->Html->link(__('Add '), ['action' => '../example/add', $idd],['class'=>'button']) ?></td>
            </tr>
            <?php endforeach; ?>
