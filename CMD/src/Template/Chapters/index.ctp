...
<body class="grey lighten-3">
  ...
  <main class="pt-5 mx-lg-5">
    <div class="container-fluid mt-5">
	    ...
	    <h3><?= __('Chapters') ?><?= $this->Html->link(__('Add '), ['action' => '../chapters/add'],['class'=>'btn btn-primary']) ?></h3>
...
<table id="dtMaterialDesignExample" class="table table-striped table-bordered" cellspacing="0" width="100%">
		...
		<?php foreach ($notications as $notication): ?>
            <tr>
			  <?php $idd = $notication['id'];  $topic_id = $notication['lesson_id']; ?>
			   <td><?php echo $notication['id']; ?></td>
			    <td><?php echo $this->Custom->getlesson($topic_id); ?></td>
                <td><?php echo $notication['code']; ?></td>
				 
                <td><?php echo $notication['name']; ?></td>
               
                  
		<td><?php echo $notication['chapterorder']; ?></td>
                <td class="actions">
