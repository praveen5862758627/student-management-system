 ...
 * @var \App\Model\Entity\Icc[]|\Cake\Collection\CollectionInterface $iccs
 */
?>
...
<div class="iccs index large-9 medium-8 columns content">
    <h3><?= __('Iccs') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($iccs as $icc): ?>
            <tr>
                <td><?= $this->Number->format($icc->id) ?></td>
                <td><?= $icc->has('industry') ? $this->Html->link($icc->industry->name, ['controller' => 'Industry', 'action' => 'view', $icc->industry->id]) : '' ?></td>
                <td><?= h($icc->code) ?></td>
                <td><?= h($icc->name) ?></td>
