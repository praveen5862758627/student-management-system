<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Depcomp $depcomp
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Depcomp'), ['action' => 'edit', $depcomp->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Depcomp'), ['action' => 'delete', $depcomp->id], ['confirm' => __('Are you sure you want to delete # {0}?', $depcomp->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Depcomps'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Depcomp'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Deps'), ['controller' => 'Deps', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Dep'), ['controller' => 'Deps', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="depcomps view large-9 medium-8 columns content">
    <h3><?= h($depcomp->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Dep') ?></th>
            <td><?= $depcomp->has('dep') ? $this->Html->link($depcomp->dep->name, ['controller' => 'Deps', 'action' => 'view', $depcomp->dep->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Topic Code') ?></th>
            <td><?= h($depcomp->topic_code) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Lesson Code') ?></th>
            <td><?= h($depcomp->lesson_code) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Level Code') ?></th>
            <td><?= h($depcomp->level_code) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Score') ?></th>
            <td><?= h($depcomp->score) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($depcomp->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($depcomp->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($depcomp->modified) ?></td>
        </tr>
    </table>
</div>
