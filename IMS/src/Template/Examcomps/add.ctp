...
<div class="examcomps form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Add Examcomp') ?></legend>
		
		 
        <?php
            echo $this->Form->control('examschedule_id', ['options' => $examschedule]);
			?>
		...
		<!--<select name="lesson_code" id="lesson_code-id" required>
		<?php
		foreach ($topiccodeslesson as $k => $v) { ?>
			
			<option value="<?php  echo $v['tcodelesson']; ?>"><?php  echo $v['tcodelesson']; ?></option>
			<?php
		...
		<select name="level_code" id="level_code" required  >
            ...
            //echo $this->Form->control('topic_code' , ['options' => $topiccodes]);
