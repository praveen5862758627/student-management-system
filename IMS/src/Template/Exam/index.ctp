 ...
 * @var \App\Model\Entity\Exam[]|\Cake\Collection\CollectionInterface $exam
 */
?>
...
<div class="exam index large-9 medium-8 columns content">
    <h3><?= __('Exam') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
				...
				<th scope="col"><?= $this->Paginator->sort('Examschedule') ?></th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td><!--<?= $this->Number->format($exam->company_id) ?> --> <?= $this->Custom->companyname($exam->company_id); ?></td>
                <td><?= h($exam->code) ?></td>
                <td><?= h($exam->name) ?></td>
				...
				<td> <?= $this->Html->link(__('Add'), ['action' => '../examschedule/add', $exam->id],['class'=>'button']) ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
